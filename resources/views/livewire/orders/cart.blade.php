                    ...
                    @foreach ($cart['items'] as $item)
                        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
                            <div class="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0" data-cart-item>
                                ...
                                <label for="counter-input-{{ $item->slug }}" class="sr-only">Choose quantity:</label>
                                <div class="flex items-center justify-between md:order-3 md:justify-end">
                                    <div class="flex items-center">
                                        <button type="button" wire:click="decrementQuantity('{{ $item->slug }}')"
                                        ...
                                        <input type="text" id="counter-input-{{ $item->slug }}" 
                                            wire:model.live="quantities.{{ $item->slug }}"
                                            class="w-10 shrink-0 border-0 bg-transparent text-center text-sm font-medium text-gray-900 focus:outline-none focus:ring-0 dark:text-white"
                                            placeholder="" value="{{ $quantities[$item->slug] ?? 1 }}" required />
                                        <button type="button" wire:click="incrementQuantity('{{ $item->slug }}')"
                                            class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-300 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700">
                                            <svg class="h-2.5 w-2.5 text-gray-900 dark:text-white" aria-hidden="true"
                                    ...
                                    <div class="text-end md:order-4 md:w-32">
                                        <p class="text-base font-bold text-gray-900 dark:text-white">Rp {{ number_format($item->harga * ($quantities[$item->slug] ?? 1), 0 , ',', '.') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <button wire:click="removeItem('{{ $item->slug }}')"
